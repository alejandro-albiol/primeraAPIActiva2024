...
userRouter.get("/", async (req: Express.Request, res: Express.Response) => {
    ...
    let statusCode = 200;
    if (!result.success && result.rows_affected === 0) statusCode = 404;
    if (!result.success && !("rows_affected" in result)) statusCode = 500;
...
userRouter.post("/", async (req: Express.Request, res: Express.Response) => {
    const user: User = { user_name: req.body.user_name, name: req.body.name, first_surname: req.body.first_surname, email: req.body.email, password: req.body.password };
